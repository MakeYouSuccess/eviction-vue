<template>
  <v-card
    class="secondary--text pa-6 d-flex flex-column"
    @close-dialog="close"
  >
    <v-btn
      color="info"
    ...
    >
      ...
      >
        ...
        <template
          ...
          slot-scope="data"
        >
          <span class="secondary--text">{{ displayPropertyInfo(data.item) }}</span>
        </template>
        <template
          slot="item"
          slot-scope="data"
...
export default {
  ...
  created(){
    this.$http.get(`${process.env.VUE_APP_URL}/client/cases`, {
      params: {
