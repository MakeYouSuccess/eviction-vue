<template>
  ...
  >
    ...
    >
      ...
      >
        <div class="mb-4">
          <div class="custom-title pb-4">
            Create an Account
          </div>
          <div class="text-left">
            To save your document, create an account. Once you are registered, you'll have full access to the
            document, along with a number of other exclusive features and tools.
          </div>
        </div>
      ...
      <div class="mb-6">We’ll revolutionize the way you manage your properties and eviction process. Fast. Simple. Painless. Affordable.</div> -->
        ...
        <div class="my-6">
          ...
          >
            <template v-slot:prepend-inner>
              <i
                :class="errorMsg.type==='email'? 'accent--text':'info--text'"
        ...
        <div>
          ...
          >
            <template v-slot:prepend-inner>
              <i
                :class="errorMsg.type==='password'? 'accent--text':'info--text'"
    ...
    computed: {
      errorMsg(){
        ...
        if (err=== 'PasswordStrengthError: Password is too weak'){
          return {type: 'password', msg: 'Your password must contain each of the requirements listed below.'}
        }
        if (err === ''){
          //no error
          return {type: '', msg: err}
    ...
    methods: {
      ...
      async signup(){
        ...
        if (localStorage.getItem('client')){
          signupDetails['user'] = JSON.parse(localStorage.getItem('client'))
        }
        ...
        await this.$http.post(`${process.env.VUE_APP_URL}/properties/save`, {
        ...
        if (localStorage.getItem('document')){
          ...
          await this.$http.post(`${process.env.VUE_APP_URL}/${docType}`, {
                docInfo: doc,
                clientId: this.clientId,
            }).then(r => r.data)
            .then(data=>{
